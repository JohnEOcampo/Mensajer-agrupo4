...
def activate():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT * FROM activationlink WHERE challenge = ? AND state = ? and CURRENT_TIMESTAMP BETWEEN crearted AND validuntil',  (number, utils.U_UNCONFIRMED)
            ).fetchone()
...
def register():
    try:
        ...
        if request.method == 'POST':    
            ...
            if db.execute("SELECT id FROM user WHERE user = ?", (username,)).fetchone() is not None:
